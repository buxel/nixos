{ config, pkgs, lib, ... }: {
  ...
  age.secrets."blobfuse-yaml".path = config.age.secretsDir + "/blobfuse.yaml"; # blobfuse inists on a ".yaml extension. This messes up secrets handling with "nixos secrets", as dots indicate a hierarchy level in nix.
